...
async def create_completion(request: CompletionRequest, raw_request: Request):
    # logger.info("<yx> create_completion request: %s", request)
    # request.model = "/home/yx/models/Qwen/Qwen3-8B-xft"
    logger.info("<yx> create_completion request: %s", request)
    # logger.info("<yx> raw_request: %s", raw_request.url)
    generator = await openai_serving_completion.create_completion(
        request, raw_request)
    # logger.info("<yx> create_completion generator: %s", generator)
    if isinstance(generator, ErrorResponse):
        return JSONResponse(content=generator.model_dump(),
                            status_code=generator.code)
...
def run_server(args, **uvicorn_kwargs) -> None:
    ...
    # logger.info("<yx> host: %s, port: %s, root_path: %s",args.host, args.port, args.root_path)
    uvicorn.run(app,
                host=args.host,
                port=args.port,
